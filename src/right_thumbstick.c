...
void RThumbstick__report_axial(
    ...
    else right_thumbstick_report_axis(self->left.actions[0], -constrain(pos.x, -1, 0));
    //// Right.
    if (!hid_is_axis(self->right.actions[0])) self->right.report(&self->right);
    else right_thumbstick_report_axis(self->right.actions[0], constrain(pos.x, 0, 1));
    //// Up.
    if (!hid_is_axis(self->up.actions[0])) self->up.report(&self->up);
    else right_thumbstick_report_axis(self->up.actions[0], -constrain(pos.y, -1, 0));
    //// Down.
...
void RThumbstick__report(RThumbstick *self) {
    ...
    if (rts_offset_x == 0 && rts_offset_y == 0) return;
    // Get values from ADC.
